...
class GetIP extends JFrame
{
	...
	GetIP(Frameeg frameeg)
		...
		try
			{ 
				c=getContentPane(); //this method retrievescontent pane //layer so that you can place objects on it
				c.setLayout(null);
...
public void addTClient(TClient tc)//frame dikhna chalu hoga ab
{
...
getIP.dispose();//sahi raha to pehle getIP ke frame ko close kiya
addMouseMotionListener(this);//TClient ka object pass kar diya
...
is.read(length);//length read ki joki byte form me hai
int byteArrayLength=(length[3] & 0xFF | (length[2] & 0xFF)<<8 | (length[1] & 0xFF)<<16 | (length[0] & 0xFF)<<24);//byte se int me convert kari
...
TClient.this.repaint();//repaint se update method call hogi
socket.close();
...
OutputStreamWriter outputStreamWriter=new OutputStreamWriter(osw);
//System.out.println(e.getX()+"   "+e.getY());
String gh="MP_"+e.getX()+"_"+e.getY()+""+e.getButton()+"#";
//System.out.println(gh);
outputStreamWriter.write(gh);
outputStreamWriter.flush();
byte b[]=new byte[1];
...
OutputStreamWriter outputStreamWriter=new OutputStreamWriter(osw);
//System.out.println(e.getX()+"   "+e.getY());
String gh="MR_"+e.getX()+"_"+e.getY()+""+e.getButton()+"#";
//System.out.println(gh);
outputStreamWriter.write(gh);
outputStreamWriter.flush();
byte b[]=new byte[1];
...
OutputStreamWriter outputStreamWriter=new OutputStreamWriter(osw);
//System.out.println(e.getX()+"   "+e.getY());
String gh="MC_"+e.getX()+""+e.getY()+""+e.getButton()+"#";
//System.out.println(gh);
outputStreamWriter.write(gh);
outputStreamWriter.flush();
byte b[]=new byte[1];
